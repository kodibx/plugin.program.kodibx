...
def index():
    ...
    else: addDir('None', 'installation de KODI', themeit=THEME4)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
    addDir ('installation de KODI'        ,'installation de KODI',   icon=ICONBUILDS,   themeit=THEME1)
        ...
        if len(match) >= 1:
            if SEPERATE == 'true':
                ...
                if count17 > 0:
                    ...
                    addFile('%s Version de type krypton disponible pour l\'installation (%s)' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
                    if SHOW17 == 'true':
                        for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                            if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                            if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def systemInfo():
    ...
    for folder in sorted(fold, key = lambda x: x):
        ...
        if os.path.exists(xml):
            ...
            elif not (prov[0]).find('installation de KODI') == -1: picture.append(foldername)
...
def buildInfo(name):
    if wiz.workingURL(BUILDFILE) == True:
        if wiz.checkBuild(name, 'url'):
            ...
            msg += "[COLOR %s]Votre version de Kodi:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, kodi)
...
elif mode=='installation de KODI'         : buildMenu()
